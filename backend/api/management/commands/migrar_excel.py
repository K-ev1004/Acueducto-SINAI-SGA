...
import openpyxl
from api.models import Suscriptor, PeriodoLectura, Lectura, Factura, Pago, ConfiguracionGeneral
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for row in data_rows:
            ...
            valor_m3 = _dec(row[12])        # Col M = VALOR M. CUB
            valor_consumo = _dec(row[13])   # Col N = VALOR CONSUMO MES
            subsidio = _dec(row[14])        # Col O = SUBSIDIO
            aseo = _dec(row[15])            # Col P = ASEO
            total = _dec(row[16])           # Col Q = TOTAL A PAGAR MES
            abono = _dec(row[17])           # Col R = ABONO A LA DEUDA
            recibido = _dec(row[18])        # Col S = TOTAL RECIBIDO
            saldo = _dec(row[19])           # Col T = NUEVO SALDO
            deuda_acum = _dec(row[5])       # Col F = DEUDA ACU.
